...
public class LsintApp extends JPanel
    ...
    public LsintApp(NewsDataset dataset) {
        // Create an instance
    ...
    private void createNodes(DefaultMutableTreeNode top, NewsOracle newsOracle) {
        ...
        java.util.List<NewsCategory> allTopics = newsOracle.getTopics();
        ...
        if ( showClustersOnly() ) {
            if (nClusters > 0) {
                ...
                java.util.List<NewsStoryGroup> clusters = newsOracle.getStoryGroups();
                ...
                for(NewsStoryGroup sg :  clusters) {
                    ...
                    java.util.List<NewsStory> newsStories = sg.getStories();
            ...
            for(NewsCategory newsCategory : allTopics) {
                java.util.List<NewsStoryGroup> clusters = newsOracle.getStoryGroupsByTopic(newsCategory);
                ...
                if ( nClusters > 0 ) {
                    ...
                    java.util.List<NewsStory> newsStories = newsOracle.getStoriesByTopic(newsCategory);
                    NewsStory.sortByTitle(newsStories);
    ...
    public static void createAndShowUI(final LsintApp ui) {
...
//            	}
    ...
    public static void main(String[] args) {
        NewsDataset loadedDataset;
        try {
            loadedDataset = NewsDatabase.loadDataset("c:/temp/ch2.dat");
        } catch (IllegalArgumentException e) {
            // Por si acaso no teniamos ya creada la base datos
            // despues de haber hecho uso del crawler
            loadedDataset = new FileListNewsDataset("DefaultDS");
            loadedDataset.setDocumentDir("C:/iWeb2/data/ch07/all");
            loadedDataset.setTopTerms(15);
            loadedDataset.loadTopics();
            loadedDataset.loadStories();
        }

        LsintApp ui = new LsintApp(loadedDataset);

        LsintApp.createAndShowUI(ui);
    }
